...
GCD更倾向于粗粒度，它关注于三个点①任务②将任务放入队列③等待完成，对于这中间的过程都不再参与或者说不再感兴趣了,尽管使用barrier、group、semaphore这些技术和dispatch_suspend与dispatch_resume函数进行执行顺序或执行流程的过程，但都是基于队列的宏观控制，无法细微到任务内部。
<br/><br/>
