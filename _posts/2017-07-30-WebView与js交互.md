...
另外一种方式是关注于打破强引用。这里用的是YYKit中的一个弱代理类<a href="https://github.com/ibireme/YYKit/blob/master/YYKit/Utility/YYWeakProxy.m" target='blank'>YYWeakProxy</a>（这个类与YYKit框架零耦合，可单独使用），它是NSProxy的子类，用来代理其他对象，它内部保存一个属性用来弱引用被代理的类，同时将所有的消息调用转发给被代理对象，因此被代理类的任何方法都能按照它本来的执行进行响应。
